 ...
 * @author Sione
 */
public final class QuickSortInd extends QuickSort
{

    private QsortIndicesWithIndices matrixIndices;

    // //////////////////////////////////////////////////////////////////////////
    public QuickSortInd(Indices A)
    {
        this(A, null);
    }

    public QuickSortInd(Indices A, Dimension dim)
    {
        this(A, dim, false);
    }

    public QuickSortInd(Indices A, Dimension dim, boolean computeSortedIndex)
    {
     ...
     * ' Y = sort(X) '
     */
    public QuickSortInd(Indices A, boolean computeSortedIndex, boolean sortedCopy)
    {
        this(A, null, computeSortedIndex, sortedCopy);
    }

    /**
     * Creates Mat new instance of QuickSortInd
     */
    public QuickSortInd(Indices A, Dimension dim, boolean computeSortedIndex, boolean sortedCopy)
    {
        super();
        if (A == null)
        {
            String msg = "Parameter \"A\" must be non-null.";
            throw new ConditionalRuleException("QuickSortInd", msg);
        }
        ...
        if (dim == null)
        {
            if (A.isVector())
            {
                ...
                if (this.computeSortedIndex)
                {
                    ...
                    // sortedIndices.printInLabel("sortedIndices",0);
                }
                this.matrixIndices = new QsortIndicesWithIndices((Indices) this.sortedObject, this.sortedIndices, null,
            ...
            else
            {
                ...
                this.matrixIndices = new QsortIndicesWithIndices((Indices) this.sortedObject, this.sortedIndices,
        ...
        else
        {
            ...
            if (this.computeSortedIndex)
            {
               ...
               */
            this.matrixIndices = new QsortIndicesWithIndices((Indices) this.sortedObject, this.sortedIndices, dim,
                    this.computeSortedIndex);
            sortMatrix(this.matrixIndices);
        }
    ...
    protected void sortMatrix(Object sortedObj)
    {
        ...
        QsortIndicesWithIndices AB = (QsortIndicesWithIndices) sortedObj;
        if (AB.isVector())
        {
            if (AB.isRowVector())
            {// row vector
    ...
    {
        sort((QsortIndicesWithIndices) sortedObj);
     ...
     */
    private void sort(QsortIndicesWithIndices AB)
    {
        String msg = "";
        if (AB == null)
        {
     ...
     ***********************************************************************/
    private void quicksort(QsortIndicesWithIndices AB, int numRowCols)
    {
    ...
    private void quicksort(QsortIndicesWithIndices AB, int left, int right, int numRowCols)
    {
        if (right <= left)
        {
            return;
    ...
    private int partition(QsortIndicesWithIndices AB, int left, int right, int numRowCols)
    {
        int i = left - 1;
        int j = right;
        while (true)
    ...
    private void exch(QsortIndicesWithIndices AB, int i, int j, int numRowCols)
    {
        exchanges++;
    ...
    // shuffle the array b[]
    private void shuffle(QsortIndicesWithIndices AB, int numRowCols)
    {
        int N = 0;
    ...
    private static void mainExample()
    {
        ...
        Indices Mat = genTestMatrix();// .getRowAt(0).transpose();//genRandMatrix(M,
                                      // N).round();
        /*
        ...
        Dimension dimMat = Dimension.ROW;// Dimension.COL;
        ...
        QuickSort sort = new QuickSortInd(Mat, dimMat, true, true);
        long stop = System.currentTimeMillis();
        System.out.println("===END===");

        Indices A = (Indices) sort.getSortedObject();// new
        ...
        Sort sortMat = new SortInd(Mat);
        Indices C = (Indices) sortMat.getSortedObject();
    ...
    static Indices genRandMatrix(int M, int N)
    {
        return genRandMatrix(M, N, 100.0);
    }

    static Indices genRandMatrix(int M, int N, double scale)
    {
        Matrix Mat = Matrix.random(M, N);
        Mat = Mat.arrayTimes(scale);
        return Mat.toIndices();
    }

    private static Indices genTestMatrix()
    {
        int[][] a =
        {
                {
    ...
    {
        Indices Mat = genTestMatrix();
        Mat.printInLabel("Mat1", 0);
        QuickSort sort = new QuickSortInd(Mat, Dimension.ROW, true);
        long stop = System.currentTimeMillis();
    ...
    static void example3()
    {
        ...
        Indices OrInd = new Indices(OR);
        QuickSort sort = new QuickSortInd(OrInd, true, true);// QuickSortInd(Indices
                                                             // A, boolean
                                                             // computeSortedIndex,
                                                             // boolean
                                                             // sortedCopy)
        Indices sortObj = (Indices) sort.getSortedObject();
        sortObj.printInLabel("sortObj");
        Indices sortInd = sort.getIndices();
        sortInd.printInLabel("sortInd");
    }
