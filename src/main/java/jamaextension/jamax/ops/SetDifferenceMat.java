 ...
 */
public class SetDifferenceMat extends SetDifference
{

    public SetDifferenceMat(Matrix A, Matrix B)
    {
        super(A, B);
        findDiff();
    }

    public SetDifferenceMat(Matrix A, Matrix B, Dimension flag)
    {
        super(A, B, flag);
        findDiff();
    }
    ...
    {
        Matrix A = (Matrix) this.getFirstObj();
        Matrix B = (Matrix) this.getSecondObject();
        ...
        {// ////////////////////////////////////////////////////////
            ...
            {// If B is empty, invoke UNIQUE to remove duplicates from A.
                ...
                this.diffObject = (Matrix) obj[0];
                this.maxIndices = (Indices) obj[1];
                return;
            }
            else if (numelA == 1)
            {
                if (!B.toArrayList().contains(new Double(A.start())))
                {
                    this.diffObject = A.getAsMatrix(0, 0);
            ...
            { // General case
              ...
              // a = a(:);
                ...
                ArrayList<Double> list = B.toArrayList();
                // int lenList = list.size();
                int lenA = A.length();
                ArrayList<Integer> index = new ArrayList<Integer>();
                for (int i = 0; i < lenA; i++)
                {
                    if (!list.contains(new Double(A.get(i, 0))))
                    {
    ...
    public static void main(String[] args)
    {
        ...
        SetDifference SD = new SetDifferenceMat(A, B);
        Matrix D = (Matrix) SD.getDiffObject();
