...
public class Dcs_counts
{
    ...
    private static int[] init_ata(Sparse AT, int[] post, int[] w)
    {
        ...
        for (i = 0; i < m; i++)
        {
            ...
            next[next_offset + i] = head[head_offset + k]; /*
                                                            * place row i in
                                                            * linked list k
                                                            */
     ...
     */
    public static int[] cs_counts(Sparse A, int[] parent, int[] post, boolean ata)
    {
        ...
        {
            j = post[k];
            delta[j] = (first[first_offset + j] == -1) ? 1 : 0; /*
                                                                 * delta[j]=1 if
                                                                 * j is a leaf
                                                                 */
        ...
        for (k = 0; k < n; k++)
        {
            ...
            for (J = HEAD(k, j, head, head_offset, ata); J != -1; J = NEXT(J, next, next_offset, ata)) /*
                                                                                                        * J
                                                                                                        * =
                                                                                                        * j
                                                                                                        * for
