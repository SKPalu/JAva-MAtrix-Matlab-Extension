...
/**
 * Support routines for Sparse_demo*.java
 * 
 * @author Piotr Wendykier (piotr.wendykier@gmail.com)
 * 
 */
public class Sparse_demo
{
    ...
    private static int is_sym(Sparse A)
    {
    ...
    /* C = A + triu(A,1)' */
    private static Sparse make_sym(Sparse A)
    {
        Sparse AT, C;
        AT = Dcs_transpose.cs_transpose(A, true); /* AT = A' */
        Dcs_fkeep.cs_fkeep(AT, new Dropdiag(), null); /*
    ...
    private static void print_resid(boolean ok, Sparse A, double[] x, double[] b, double[] resid)
    {
        int i, m, n;
        if (!ok)
        {
        ...
        System.out.print(String.format("resid: %8.2e\n",
                norm(resid, m) / ((n == 0) ? 1 : (Dcs_norm.cs_norm(A) * norm(x, n) + norm(b, m)))));
    }
    ...
    public static Dproblem get_problem(String fileName, double tol)
    {
        ...
        Prob.A = A = Dcs_compress.cs_compress(T); /*
    ...
    public static boolean demo2(Dproblem Prob)
    {
        ...
        for (order = 0; order <= 1; order++) /* natural and amd(A+A') */
        {
            ...
            rhs(x, b, m); /* compute right-hand side */
            t = tic();
            ok = Dcs_cholsol.cs_cholsol(order, C, x); /*
                                                       * solve Ax=b with
                                                       * Cholesky
                                                       */
            System.out.print(String.format("time: %8.2f ms ", toc(t)));
    ...
    {
        Sparse A, C, W = null, WW, WT, E = null, W2;
