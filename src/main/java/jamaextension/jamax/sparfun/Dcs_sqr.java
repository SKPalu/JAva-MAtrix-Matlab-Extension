...
public class Dcs_sqr
{
    ...
    private static boolean cs_vcount(Sparse A, Dcss S)
    {
     ...
     */
    public static Dcss cs_sqr(int order, Sparse A, boolean qr)
    {
        int n, k, post[];
        Dcss S;
        boolean ok = true;
        ...
        {
            Sparse C = order > 0 ? Dcs_permute.cs_permute(A, null, S.q, false) : A;
            S.parent = Dcs_etree.cs_etree(C, true); /*
                                                     * etree of C'*C, where
                                                     * C=A(:,q)
                                                     */
            post = Dcs_post.cs_post(S.parent, n);
            S.cp = Dcs_counts.cs_counts(C, S.parent, post, true); /*
                                                                   * col counts
                                                                   * chol(C'*C)
                                                                   */
