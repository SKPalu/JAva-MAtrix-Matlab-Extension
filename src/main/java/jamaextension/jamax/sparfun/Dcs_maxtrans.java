...
{
    /*
     * find an augmenting path starting at column k and extend the match if
     * found
     */
    private static void cs_augment(int k, Sparse A, int[] jmatch, int jmatch_offset, int[] cheap, int cheap_offset,
            int[] w, int w_offset, int[] js, int js_offset, int[] is, int is_offset, int[] ps, int ps_offset)
    {
        ...
        while (head >= 0)
        {
            ...
            if (w[w_offset + j] != k) /* 1st time j visited for kth path */
            {
                ...
                cheap[cheap_offset + j] = p; /*
                                              * start here next time j is
                                              * traversed
                                              */
                if (found)
                {
            ...
            for (p = ps[ps_offset + head]; p < Ap[j + 1]; p++)
            {
                ...
                js[js_offset + (++head)] = jmatch[jmatch_offset + i]; /*
                                                                       * start
                                                                       * dfs at
                                                                       * column
                                                                       * jmatch
     ...
     */
    public static int[] cs_maxtrans(Sparse A, int seed) /*
                                                         * [jmatch [0..m-1];
                                                         * imatch [0..n-1]]
                                                         */
    {
        ...
        for (i = 0; i < m; i++)
            m2 += w[i];
        C = (m2 < n2) ? Dcs_transpose.cs_transpose(A, false) : A; /*
                                                                   * transpose
                                                                   * if needed
                                                                   */
        if (C == null)
