 ...
 * </pre>
 * 
 * where A is a nonsingular Zmat, B is a Zmat, and '^H' denotes the conjugate
 * transpose. Appropriate action is taken for Zmats that are Zltmats, Zutmats,
 * and Zpsdmats. If a decomposition is computed and the <a
 * href="Parameters.html"> History </a> parameter is set, then the decomposition
 * is saved for reuse.
 * <p>
 * Comments: For triangular matrices only the systems AX=B and A^HX=B are solved
 * by hard code, the other two being solved by wizardry involving transposed
 * systems. This requires the generation of new Zmats of the same size as B,
 * which is inefficient if B is, say, square. Later these methods will be
 * implemented with hard code.
 * 
 * @version Pre-alpha
 * @author G. W. Stewart
 */
