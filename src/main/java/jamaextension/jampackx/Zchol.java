 ...
 * Specifically if A is (Hermitian) positive definite then there is an upper
 * triangular matrix R with positive diagonal elements such that
 * 
 * <pre>
 *     A = R^H R
 * </pre>
...
public class Zchol
{
    ...
    /**
     * Constructs a Zchol from a Zmat A. The matrix that is actually decomposed
     * is taken from the upper triangle of $A$ and the imaginary part of its
     * diagonal is set to zero. Throws a JampackException for inconsistent
     * dimensions on failure of the algorithm to complete.
     * 
     * @param A
