...
/**
 * Print prints numbers, matrices, and arrays. All floating-point numbers are
 * printed in e format with field width w and d figures to the left of the right
 * of the decimal point. For the defaults see <a href="Parameters.html">
 * Parameters </a>.
 * 
 * @version Pre-alpha
 * @author G. W. Stewart
...
public class Print
{
        ...
        while (jl < nc)
        {
            ...
            for (int i = 0; i < nr; i++)
            {
                ...
                for (int j = jl; j < ju; j++)
                {
                    ...
                    if (A[i][j].im < 0)
                        snum = snum + " - " + DoubletoEstring(-A[i][j].im, d + 6, d) + "i";
                    else
                        snum = snum + " + " + DoubletoEstring(A[i][j].im, d + 6, d) + "i";
    ...
    /**
     * Prints a Zmat in w.d e format. This method checks to see if the Zmat is
     * real, in which case it prints only the real part.
     */
            ...
            while (jl < nc)
            {
                ...
                for (int i = 0; i < nr; i++)
                {
                    ...
                    for (int j = jl; j < ju; j++)
                    {
                        ...
                        if (A.im[i][j] < 0)
                            snum = snum + " - " + DoubletoEstring(-A.im[i][j], d + 6, d) + "i";
                        else
                            snum = snum + " + " + DoubletoEstring(A.im[i][j], d + 6, d) + "i";
                        System.out.print(snum);
                    }
    ...
    public static String ZtoEstring(Z num, int w, int d)
    {
        ...
        if (num.im < 0)
            snum = snum + " - " + DoubletoEstring(-num.im, d + 6, d) + "i";
        else
            snum = snum + " + " + DoubletoEstring(num.im, d + 6, d) + "i";
        return snum;
    }
