...
/**
 * Computes a Householder QR decomposition.Specifically, given a matrix A there
 * are is a unitary matrix U such that
 * 
 * <pre>
 * QA = R
 * </pre>
 * 
 * where R is zero below its diagonal. In constructing this decomposition, Zhqrd
 * represents Q as a product of Householder transformations with each
 * transformation represented by a Z1. R is represented by a Zutmat. Methods are
 * provided to apply the transformations to other matrices. <br>
 * Comments: The routines to postmultiply by Q are soft coded and should
...
public class Zhqrd
{
    ...
    /**
     * An array containing the generating vectors for the Householder
     * transformations.
     */
    public Z1[] U;

    /**
     * The R factor. If nrow&gt;ncol then R is square of order ncol. Otherwise R
     * has the same dimenstions as A.
     */
    ...
    public Zhqrd(Zmat A) throws JampackException
    {
        ...
        if (nrow > ncol)
        {// Chop off zeros at the bottom.
            R = new Zutmat(R.get(R.bx, R.cx, R.bx, R.cx));
        }
    }

    /**
     * Computes the product QB. Throws JampackException for inconsistent
     * dimenstions.
     * 
     * @param B
     *            A Zmat
    ...
    /**
     * Computes the product BQ. Throws JampackException for inconsistent
     * dimenstions.
     * 
     * @param B
     *            A Zmat
